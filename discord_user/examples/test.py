...
client = discord_user.Client(secret_token=secret.auth_token_discord, proxy_uri=proxy_uri, device=ClientDevice.android)
    ...
    if slash_command_message.embeds: # только с текстом
        ...
        if slash_command_message.author.id == "464272403766444044": # SD.C Monitoring
            if first_embed.image.url:
                ...
                print(f"Ответ [/up]: {slash_command_message.embeds[0].description}")
    # else:
